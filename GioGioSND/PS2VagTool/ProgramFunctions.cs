...
namespace PS2VagTool
{
    ...
    internal static class ProgramFunctions
    {
        ...
        //-------------------------------------------------------------------------------------------------------------------------------
        internal static VagEntry GetEntryFromEncoder(string inputFile, bool encodeLooped)
        // dont need the little thing at the start, also wav only
        {
            //Variables to store audio info
            short[] pcmData = null;
            int frequency = 0, channels = 0;
            ...
            vagData = SonyVag.Encode(pcmData, loopStartValue, loopEndValue, encodeLooped);
            VagEntry output = new VagEntry
            {
                sample_rate = (ushort)frequency,
                looped = encodeLooped,
                vag = vagData
            };
            return output;
